...
namespace GorillaNetworking
{
	public class PhotonNetworkController : MonoBehaviourPunCallbacks
	{
		...
		private string _gameVersion = "live1.1.1.37";
		...
		public string roomCharacters = "ABCDEFGHIJKLMNOPQRSTUVWXYX123456789";
		...
		{
			if (StartGeoTrigger == null)
			{
				yield return new WaitForSeconds(2f);
				GameObject[] array = disableOnStartup;
				foreach (GameObject obj in array)
				{
					bool active = obj.name == startLevel;
			...
			{
				StartGeoTrigger.OnBoxTriggered();
			}
		}
		...
		public void AttemptJoinPublicWithFriends(GorillaNetworkJoinTrigger triggeredTrigger)
		{
			...
			keyToFollow = PhotonNetwork.LocalPlayer.ActorNumber + PhotonNetwork.CurrentRoom.Name;
			BroadcastMyRoom(create: false);
			ProcessState(ConnectionEvent.AttemptJoinPublicRoom);
		}

		public void AttemptToFollowFriendIntoPub(string userIDToFollow, int actorNumberToFollow, string roomToFollow)
		{
			friendToFollow = userIDToFollow;
			keyToFollow = actorNumberToFollow + roomToFollow;
		...
		public void StartSearchingForFriend()
		{
			...
			StartCoroutine(SearchForFriendToJoin(friendToFollow, keyToFollow));
		}

		private IEnumerator SearchForFriendToJoin(string userID, string keyToFollow)
		{
				...
				try
				{
					...
					}, delegate(GetSharedGroupDataResult result)
					{
						foreach (KeyValuePair<string, SharedGroupDataRecord> datum in result.Data)
						{
							...
							{
								customRoomID = datum.Value.Value;
		...
		private void BroadcastMyRoom(bool create)
		{
			...
			PlayFabClientAPI.ExecuteCloudScript(new ExecuteCloudScriptRequest
			{
				...
				FunctionParameter = new
				{
					...
					RoomToJoin = PhotonNetwork.CurrentRoom.Name,
					Set = create
				}
			}, delegate
			{
