...
namespace GorillaTag
{
	...
	public class ContainerLiquid : MonoBehaviour
	{
		...
		{
			base.enabled = meshRenderer != null && meshFilter != null && spillParticleSystem != null;
		...
		protected void LateUpdate()
		{
			...
			Vector3 position2 = Vector3.Lerp(a, b, fillAmount);
			...
			Vector3 normalized = new Vector3(vector2.x, -1f, vector2.y).normalized;
			Vector3 vector3 = transform.InverseTransformDirection(normalized);
			...
			float num3 = x2 * x;
			float y = localMeshBounds.extents.y;
			Vector3 vector5 = localMeshBounds.center + new Vector3(0f, y, 0f);
			cupTopWorldPos = transform.TransformPoint(vector5);
			_ = transform.up;
			Vector3 vector6 = transform.InverseTransformDirection(Vector3.down);
			Vector3 position3 = vector5 + new Vector3(vector6.x, 0f, vector6.z).normalized * x2;
			bottomLipWorldPos = transform.TransformPoint(position3);
			float num4 = Mathf.Clamp01((position2.y - bottomLipWorldPos.y) / (num3 * 2f));
