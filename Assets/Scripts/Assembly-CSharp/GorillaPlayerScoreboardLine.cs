		...
		while (true)
		{
			try
			{
				...
				if (linePlayer != null)
				{
					...
					{
						playerVRRig = GorillaGameManager.StaticFindRigForPlayer(linePlayer).GetComponent<VRRig>();
						if (playerVRRig != null)
						{
							if (linePlayer != PhotonNetwork.LocalPlayer)
							{
					...
					if (myVoiceView == null)
					{
						myVoiceView = playerVRRig.GetComponent<PhotonVoiceView>();
					}
					if (myRecorder == null)
					{
						myRecorder = PhotonNetworkController.Instance.photonVoiceObjectPrefab.GetComponent<Recorder>();
					}
	...
	public void PressButton(bool isOn, GorillaPlayerLineButton.ButtonType buttonType)
	{
		...
		case GorillaPlayerLineButton.ButtonType.Mute:
			if (linePlayer != null && playerVRRig != null)
			{
				...
				if (playerVRRig != null)
				{
					playerVRRig.muted = ((mute != 0) ? true : false);
				}
				PlayerPrefs.Save();
				muteButton.UpdateColor();
