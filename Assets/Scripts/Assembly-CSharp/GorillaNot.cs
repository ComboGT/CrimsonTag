...
public class GorillaNot : MonoBehaviourPunCallbacks
{
	...
	private void IncrementRPCCallLocal(PhotonMessageInfo info, string rpcFunction)
	{
		if (info.Sender != null && info.Sender.UserId != null && info.SentServerTimestamp >= lastServerTimestamp)
		{
			...
			if (!userRPCCalls.TryGetValue(info.Sender.UserId, out var value2))
			{
				...
				userRPCCalls.Add(info.Sender.UserId, dictionary);
			}
			...
			if (value != null && !IncrementRPCCallLocal(in value))
			{
	...
	private bool IncrementRPCCallLocal(in RPCCallTracker rpcCallTracker)
	{
		...
		if (rpcCallTracker.RPCCalls > rpcCallLimit)
		{
			return false;
		}
		return true;
