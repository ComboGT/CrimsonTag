...
public class Mods2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Fly)
        {
            ...
            {
                GorillaLocomotion.Player.Instance.GetComponent<Rigidbody>().linearVelocity = GorillaLocomotion.Player.Instance.headCollider.transform.forward * Time.deltaTime * 36f;    
            }
            else
            {
                ...
                GorillaLocomotion.Player.Instance.GetComponent<Rigidbody>().linearVelocity = Vector3.zero;
            }
